...
function buildPython(tool: Tool, params: ToolParams, b64: string): string {
  ...
  if (tool === 'replace_text') {
    script = [
      ...
      '            x = float(tm[4])',
      '            y = float(tm[5])',
      '            fs = float(fontSize) if fontSize else 10',
      '            if fs < 4:',
      '                fs = 10',
      '            parts.append((text, x, y, fs))',
      '    page.extract_text(visitor_text=visitor)',
      '    if parts:',
      '        buf = io.BytesIO()',
      '        c = canvas.Canvas(buf, pagesize=(w, h))',
      '        for (text, x, y, fsize) in parts:',
      '            new_text = text.replace(find_text, replace_text_val)',
      '            if new_text == text:',
      '                continue',
      '            replaced_count += 1',
      ...
      '            c.rect(x - 0.5, y - fsize * 0.28, old_w + 1, fsize * 1.25, fill=1, stroke=0)',
      ...
      '    writer.write(f)',
      'print(f"Replaced {replaced_count} text fragments")',
      finish,
    ].join("\n")
  }
          ...
          <div className={styles.paramsBox}>
            {activeTool === 'replace_text' && (
              <div className={styles.paramGroup}>
                ...
                <p className={styles.paramHint}>Меняйте по одной строке за раз. Скопируйте текст точно как в документе (регистр важен). Работает только для текстовых PDF, не сканов.</p>
              </div>
            )}
